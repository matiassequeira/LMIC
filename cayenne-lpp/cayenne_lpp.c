...
void cayenne_lpp_add_digital_output(cayenne_lpp_t *lpp, uint8_t channel,
                                    uint8_t value)
{
    if((lpp->cursor + CAYENNE_LPP_DIGITAL_OUTPUT_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_analog_input(cayenne_lpp_t *lpp, uint8_t channel,
                                  float value)
{
    if((lpp->cursor + CAYENNE_LPP_ANALOG_INPUT_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_analog_output(cayenne_lpp_t *lpp, uint8_t channel,
                                   float value)
{
    if((lpp->cursor + CAYENNE_LPP_ANALOG_OUTPUT_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_luminosity(cayenne_lpp_t *lpp, uint8_t channel,
                                uint16_t lux)
{
    if((lpp->cursor + CAYENNE_LPP_LUMINOSITY_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_presence(cayenne_lpp_t *lpp, uint8_t channel,
                              uint8_t value)
{
    if((lpp->cursor + CAYENNE_LPP_PRESENCE_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_temperature(cayenne_lpp_t *lpp, uint8_t channel,
                                 float celsius)
{
    if((lpp->cursor + CAYENNE_LPP_TEMPERATURE_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_relative_humidity(cayenne_lpp_t *lpp, uint8_t channel,
                                       float rh)
{
    if((lpp->cursor + CAYENNE_LPP_RELATIVE_HUMIDITY_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
{
    if((lpp->cursor + CAYENNE_LPP_ACCELEROMETER_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_barometric_pressure(cayenne_lpp_t *lpp, uint8_t channel,
                                         float hpa)
{
    if((lpp->cursor + CAYENNE_LPP_BAROMETRIC_PRESSURE_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
void cayenne_lpp_add_gyrometer(cayenne_lpp_t *lpp, uint8_t channel, float x,
                               float y, float z)
{
    if((lpp->cursor + CAYENNE_LPP_GYROMETER_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
...
{
    if((lpp->cursor + CAYENNE_LPP_GPS_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE){
