...
static bit_t decodeFrame (void) {
    ...
    __attribute__((unused)) const char *window = (LMIC.txrxFlags & TXRX_DNW1) ? "RX1" : ((LMIC.txrxFlags & TXRX_DNW2) ? "RX2" : "Other");
...
static bit_t processJoinAccept (void) {
    ...
    __attribute__((unused)) u4_t mic  = os_rlsbf4(&LMIC.frame[dlen-4]); // safe before modified by encrypt!
...
static void engineUpdate (void) {
    ...
    __attribute__((unused)) ostime_t rxtime = 0;
    ostime_t txbeg  = 0;
    ...
    if( (LMIC.opmode & (OP_JOINING|OP_REJOIN|OP_TXDATA|OP_POLL)) != 0 ) {
    	...
    	__attribute__((unused)) bit_t jacc = ((LMIC.opmode & (OP_JOINING|OP_REJOIN)) != 0 ? 1 : 0);
        #if LMIC_DEBUG_LEVEL > 1
            if (jacc)
